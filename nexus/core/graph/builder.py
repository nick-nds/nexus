...
class GraphBuilder:
    ...
    def _pick_class_kind(
        ...
        1. Profile ``custom_bases``: an exact parent-class match wins.
        2. Profile ``custom_suffixes``: a class-name suffix match wins.
           (Suffixes are matched against the FQN tail, not the short
           name, so a profile entry like ``Handler`` matches both
           ``CreateUserHandler`` and ``Auth\LogoutHandler``.)
        3. Built-in :data:`_KIND_PRIORITY` mapping over the kinds the
           Phase 1 classifier emitted.
        4. Fallback to :attr:`NodeKind.CLASS` for unclassified classes.

        Profile-defined kinds end up as plain ``NodeKind.CLASS`` in v1
        but the original label is preserved in the node's
        ``attributes['kinds']`` for downstream consumers.
        """
        ...
        # Profile customisations only apply when no built-in kind fit.
        if parent is not None and parent in profile.custom_bases:
